...
from docbot_core.storage import BotError, FileLock, home_path
...
def parser():
    ...
    sub = graph.add_subparsers(dest="action", required=True)
    sub.add_parser("on")
    sub.add_parser("off")
...
def serve(bot, max_jobs=0):
    """Keep the loaded model between files, while releasing the work lock for graphs."""
    processed = 0
        ...
        while True:
            ...
            if result["status"] == "awaiting_assistant":
                ...
                while time.monotonic() < deadline:
                    ...
                    if not job or job["state"] not in {"awaiting_assistant", "graphing", "indexed"} or (doc and doc[0]):
                        break
                    time.sleep(.2)
                else:
                    return result | {"processed": processed, "model_released_after_wait_seconds": 120}
    ...
    try:
        ...
        if command == "setup":
            ...
            if args.action in {"on", "off"}:
                result = bot.graph_toggle(args.action == "on")
            elif args.action == "build":
                result = bot.graph_build(args.document)
            elif args.action == "next":
                result = bot.graph_next(args.owner)
            elif args.action == "commit":
                result = bot.graph_commit(read_json(args.payload), args.owner)
            elif args.action == "fail":
                result = bot.graph_fail(args.job, args.owner, args.reason)
