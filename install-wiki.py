#!/usr/bin/env python3
"""Install only Document Bot Wiki's owned skill directory; never rewrite host configuration."""
import argparse
import json
...
def install(host, user_home, update=False):
    source = Path(__file__).resolve().parent / "document-bot-wiki"
    target = user_home / (".claude/skills" if host == "claude" else ".agents/skills") / "document-bot-wiki"
    marker = target / ".document-bot-wiki-install.json"
    if target.is_symlink():
        raise ValueError(f"Refusing to overwrite a symlink: {target}")
    if target.exists() and (not update or not marker.is_file()):
        raise ValueError(f"Skill already exists: {target}. Use --update only for a previous Document Bot Wiki installation.")
    if target.exists() and marker.is_file():
        metadata = json.loads(marker.read_text(encoding="utf-8"))
        if metadata.get("package") != "document-bot-wiki":
    ...
    marker.write_text(json.dumps({"package": "document-bot-wiki", "version": VERSION}), encoding="utf-8")
    return {"host": host, "path": str(target), "version": VERSION}


def main():
    parser = argparse.ArgumentParser(description="Install Document Bot Wiki into Claude Code and/or Codex")
    ...
    try:
        ...
        print(json.dumps({"installed": results, "next": "在既有工作目錄使用 document-bot-wiki setup。"}, ensure_ascii=False))
