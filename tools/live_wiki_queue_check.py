"""Exercise real background workers, polling, FIFO and wiki gating with generated attachments."""
import json
...
def main():
    base = ROOT / ".test-runtime" / ("wiki-queue-"+uuid.uuid4().hex)
    ...
    try:
        while time.monotonic()-started < 180:
            ...
            if not inserted_d and any(j["state"] in {"indexing", "awaiting_assistant", "compiling"} for j in current):
                receipt = enqueue("D")
            ...
            if any(j["state"] == "awaiting_assistant" for j in current):
                ...
                if batch["status"] == "batch":
                    # Deliberately non-domain fixture: an empty wiki is correct here.
        ...
        output = ROOT / "verification" / "wiki"
        output.mkdir(exist_ok=True, parents=True)
        raw = json.dumps(report, ensure_ascii=False, indent=2).replace(json.dumps(str(ROOT))[1:-1], "<PROJECT_ROOT>")
        (output / "live-queue.json").write_text(raw, encoding="utf-8")
